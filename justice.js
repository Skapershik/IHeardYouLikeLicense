    ...
    if($('.b-link_button.is-licensed').length) {
        ...
        chrome.extension.sendMessage(JSON.stringify({'action': 'get', 'link': 'http://licensecrush.ddns.net/anime_episodes_info/'+name+'/'+episode+'/'}), function(data) {
            ...
            if(!$.isEmptyObject(data[1]) && (data[1]['fandub'].length + data[1]['subtitles'].length + data[1]['raw'].length)) {
                ...
                player_container = `<div class="player-container">
            ...
            let goto_episode = function(episode_num) {
                ...
                chrome.extension.sendMessage(JSON.stringify({'action': 'get', 'link': 'http://licensecrush.ddns.net/anime_episode_info/'+name+'/'+episode+'/'}), function(ep_data) {
...
$(document).ready(function() {
    ...
    //console.log($('.subheadline').first().text('kokus kekus'))
});
